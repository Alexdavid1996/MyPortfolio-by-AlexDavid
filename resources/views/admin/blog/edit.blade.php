...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.blog.update', $post) }}" enctype="multipart/form-data" class="space-y-4 max-w-3xl">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $post->title) }}" class="mt-1 w-full rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
            @error('title')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Slug</label>
            <input type="text" name="slug" value="{{ old('slug', $post->slug) }}" class="mt-1 w-full rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
        ...
        <div>
            ...
            <textarea name="excerpt" class="mt-1 w-full rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600">{{ old('excerpt', $post->excerpt) }}</textarea>
        ...
        <div>
            ...
            @if($post->cover_image_url)
                <img src="{{ asset($post->cover_image_url) }}" alt="Current feature image" class="mt-2 w-32 h-32 object-cover rounded">
            @endif
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Meta Title</label>
            <input type="text" name="meta_title" value="{{ old('meta_title', $post->meta_title) }}" class="mt-1 w-full rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
        ...
        <div>
            ...
            <textarea name="meta_description" class="mt-1 w-full rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600">{{ old('meta_description', $post->meta_description) }}</textarea>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Auto slug/meta-title from Title — but DO NOT overwrite slug if user touched it
  title.addEventListener('input', debounce(e => {
    const val = e.target.value.trim();
    if (!val) {
      if (!slugTouched) slug.value = '';
      if (!metaTouched) metaTitle.value = '';
      return;
    }
